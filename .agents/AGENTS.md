...
- **Résolution Déterministe des Processus OS** : Interdiction de reposer sur le prompt système ou des comparaisons chaînes strictes (`==`) pour cibler des processus ou fenêtres. Toute action système (`ouvrir_application`, `fermer_application`, recherche) doit utiliser un module de résolution Python déterministe partagé qui harmonise le vocabulaire entre ouverture et fermeture, gère la troncature du noyau Linux `/proc/<pid>/comm` (15 caractères max), et associe dynamiquement les fichiers `.desktop` et alias système sans liste blanche fixe codée en dur.